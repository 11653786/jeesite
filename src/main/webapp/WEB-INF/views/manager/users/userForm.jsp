...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/users/user/">客户管理列表</a></li>
		<li class="active"><a href="${ctx}/users/user/form?id=${user.id}">客户管理<shiro:hasPermission name="users:user:edit">${not empty user.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="users:user:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/users/user/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			<label class="control-label">支付宝标志：</label>
			<div class="controls">
				<form:input path="alipayid" htmlEscape="false" maxlength="255" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="users:user:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
