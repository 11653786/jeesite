...
<ul class="nav nav-tabs">
    ...
    <li class="active"><a href="${ctx}/orders/orders/form?id=${orders.id}">订单管理<shiro:hasPermission
            name="orders:orders:edit">${not empty orders.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission
            name="orders:orders:edit">查看</shiro:lacksPermission></a></li>
</ul>
<br/>
<form:form id="inputForm" modelAttribute="orders" action="${ctx}/orders/orders/save" method="post"
    ...
    <div class="control-group">
        <c:forEach items="${orderGoods}" var="orderGood">
            商品名称:${orderGood.productName},商品价格:${orderGood.productPrice},商品数量:${orderGood.productNum}<br>
        </c:forEach>
    </div>
